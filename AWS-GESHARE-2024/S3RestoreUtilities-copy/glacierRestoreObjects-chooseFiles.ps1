...
##import-moduleAWS.Tools.StorageGateway 
...
$targetKey    = get-s3object -BucketName $targetBucket.BucketName
...
try{
    $s3BucketItems = (get-s3version -bucketname $bucketName -Prefix $bucketPrefix -Region $sGRegion -Delimiter "/").Versions | Where-Object {$_.LastModified -le [DateTime]$compareDate}
...
while($tempBucketItems.IsTruncated -eq "true"){
    ...
    try{
        ...
        $s3BucketItems += (get-s3version -bucketname $bucketName -Prefix $bucketPrefix -Region $sGRegion -KeyMarker $nextKey -Delimiter '/').Versions | Where-Object {$_.LastModified -le [DateTime]$compareDate}
...
if($s3BucketItems.length -eq 0){
    $wshell.Popup("No objects available to restore in bucket $bucketName/$bucketPrefix",0,"Information",64+0)
    write-log -Message "No objects available to restore in bucket $bucketName/$bucketPrefix" -Severity Information
    break
}
...
# Creates GUI representation of objects available to restore
...
# Conditional to determine if files were selected to restore
if($selectArray.Length -ge 1){
    ...
    # Initiate runspace pool for running multiple threads
    $MaxThreads   = [math]::Ceiling($selectArray.Count)
    $SessionState = [System.Management.Automation.Runspaces.InitialSessionState]::CreateDefault()
    $RunspacePool = [runspacefactory]::CreateRunspacePool(1, $MaxThreads, $SessionState, $Host)
    $RunspacePool.Open()
                ...
                while($workingArray.length -ge 1){
                    ...
                    foreach ($a1 in $workingArray){
                        ...
                        try{
                            ...
                            Write-Host "Retrieve object $a1.key " -ForegroundColor Yellow
                            }
                        catch{
                        }
