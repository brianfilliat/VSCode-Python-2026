...
if($nextKey -ne ""){
    try{
        $s3BucketItems += (get-s3version -bucketname $bucketName -Prefix $bucketPrefix -Region $sGRegion -KeyMarker $nextKey).Versions | Where-Object {$_.LastModified -lt [DateTime]$compareDate -and $_.IsDeleteMarker -eq "True"}
        write-log -Message "Object metadata retreival for objects with Delete Markers from S3 Bucket @bucketName before $compareDate successful" -Severity Information
    }
    catch
    {
...
if($s3BucketItems.length -eq 0){
    ...
    write-log -Message "No objects with active delete markers located in specified S3 bucket $bucketName" -Severity Information
    break
}
...
$answer = $wshell.Popup("Do you want to remove the listed delete markers?",0,"Question",64+4)

# If yes, removes delete markers, creates log file of objects 
if($answer -eq 6){
...
# Third Loop - Loops through array of arrays to submit 1000 item Key and Version data
    foreach($keyVerSet in $keyVerArray){
        ...
        try{
            Remove-S3Object -BucketName $bucketName -KeyAndVersionCollection $keyVerSet  -region $sGRegion -Force
