...
#                       then launch the webpage for the user to input CFT details.  The script
#                       will then listen on port 5001 for the webpage form submission, ingest the
#                       user input, generate the CFT based on the input, close the port listener,
#                       and display the location of the template in the web browser.
#    System Requirements: Chrome "Enable Local File Links" plugin to see result file location
#                         If your system already has a listener on port 5001, please change the listening port.
##########

# User Input form content
$website = @"
<HTML>
    <HEAD>
...
# Explort User Input form to local temporary file
$website | out-file -FilePath "./temp.html"

# Open webpage
Invoke-Expression "./temp.html"

# Create and start HTTP listener on port 5001
$httpListener = New-Object System.Net.HttpListener
$httpListener.Prefixes.Add('http://localhost:5001/')
$httpListener.Start()

# Main loop - keep listener up until user input collected and listener closed
while($httpListener.IsListening){

    $context = $httpListener.GetContext()
    $request = $context.Request

    # IO Stream reader to read user input from User Input Form
    $Reader = New-Object System.IO.StreamReader($request.InputStream)
    $inputs = $Reader.ReadToEnd()

    # Place User Input into array for processing
...
# Conditional to determine if Template is for GOV or NON-GOV Cloud Space
...
'@
            ...
            # Create file and folder name for CFT export
            $folderName = $stackNameText.replace("-","")
            $filename = "C:\temp\$folderName\" + $stackNameText + ".yml"

            # Test for folder availability and create if not present
            if(!(test-path -Path "C:\temp\$folderName")){
                New-Item -ItemType Directory -Path "C:\temp\$folderName"
            }

            # Create new empty yml file
            New-Item -ItemType File -Path $filename

            # Update contents of yml file to contain CFT data from script
