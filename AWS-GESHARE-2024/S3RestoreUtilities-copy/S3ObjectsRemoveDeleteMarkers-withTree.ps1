...
import-Module -Name AWS.Tools.StorageGateway
...
try{
    $s3BucketItems = (get-s3version -bucketname $bucketName -Prefix $bucketPrefix -Region $sGRegion.region).Versions | Where-Object {$_.LastModified -lt [DateTime]$compareDate -and $_.IsDeleteMarker -eq "True"}
...
try{
    $tempBucketItems = get-s3version -bucketname $bucketName -Prefix $bucketPrefix -Region $sGRegion.region
    write-log -Message "Object metadata retreival for objects with Delete Markers from S3 Bucket @bucketName before $compareDate successful" -Severity Information
}
catch{
    write-log -Message "Object metadata retreival for objects with Delete Markers from S3 Bucket @bucketName failed: $_" -Severity Error
...
while($tempBucketItems.IsTruncated -eq "true"){
    ...
    try{
        $nextKey = (get-s3version -bucketname $bucketName -Prefix $bucketPrefix -Region $sGRegion.region -KeyMarker $nextKey).NextKeyMarker
        write-host $nextKey
        $s3BucketItems += (get-s3version -bucketname $bucketName -Prefix $bucketPrefix -Region $sGRegion.region -KeyMarker $nextKey).Versions | Where-Object {$_.LastModified -lt [DateTime]$compareDate-and $_.IsDeleteMarker -eq "True"}
        $tempBucketItems = get-s3version -bucketname $bucketName -Prefix $bucketPrefix -Region $sGRegion.region -KeyMarker $nextKey
        write-host $nextKey
        write-log -Message "Object metadata retreival for objects with Delete Markers from S3 Bucket @bucketName before $compareDate successful" -Severity Information
    }
    catch
...
if($s3BucketItems.length -eq 0){
    $wshell.Popup("No objects with delete markers present in s3 bucket $bucketName",0,"Information",64+0)
    write-log -Message "No objects with delete markers present in s3 bucket $bucketName" -Severity Information
    break
}
...
foreach($entry in $workingArray){
    ...
    $resultsObject = New-Object -TypeName System.Management.Automation.PSObject -Property ([ordered]@{
        ...
        "File Name"     = $entry.key.substring($entry.key.lastindexof("/")+1);
        "Location"      = "S3://" + $entry.BucketName + $path;
        "Last Modified" = [DateTime]$entry.LastModified;
        "Version ID"    = $entry.VersionId;
        "Is Latest"     = $entry.IsLatest;
        "Delete Marker" = $entry.IsDeleteMarker
    })
...
# Creates GUI representation of objects from which to remove delete markers and outputs same in CSV to outputList location
$restoreArray | Sort-Object Location | Out-GridView 
$restoreArray | export-csv -Path $outputList

# Creates GUI box to ask for approval to remove delete markers from displayed list
$answer = $wshell.Popup("Do you want to restore all listed objects?",0,"Question",64+4)

# If yes, initiates restore activities for all objects
if($answer -eq 6){

    # Third Loop - Loops through array of arrays to submit 1000 item Key and Version data
    foreach($keyVerSet in $keyVerArray){
    
        try{
            Remove-S3Object -BucketName $bucketName -KeyAndVersionCollection $keyVerSet  -region $sGRegion.region -Force
...
try{
    $sGateway = Get-SGGateway -Region $sGRegion.region | out-gridview -OutputMode Multiple -Title "Select Storage Gateway to Refresh."
...
try{
    $sGatewayShare = get-sgfilesharelist -GatewayARN $sGateway.GatewayARN -region $sGRegion.region | out-gridview -OutputMode Multiple -Title "Select share on Storage Gateway Selected to refresh."
...
try{
    Invoke-SGCacheRefresh -FileShareARN $sGatewayShare.FileShareARN -region $sGRegion.region -FolderList $cacheRefreshList
